...
celltype_union = sort(unique(c(as.character(umap_wt$celltype),
                                as.character(umap_ko$celltype))))
palette_use = setNames(celltype_palette[celltype_union], celltype_union)

umap_wt$celltype = factor(umap_wt$celltype, levels = celltype_union)
umap_ko$celltype = factor(umap_ko$celltype, levels = celltype_union)

scale_celltype = scale_colour_manual(
  breaks = celltype_union,
  values = palette_use,
  drop   = FALSE
)
...
base_theme = theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        ...
        legend.position = "none")           # = turn legend OFF here
...
dummy_legend_plot = ggplot(
  data.frame(x = 1, y = 1, celltype = celltype_union),
  aes(x, y, colour = celltype)
) +
  geom_point() +                       # one point per level
  scale_celltype +
  labs(colour = NULL) +                # no title
  guides(colour = guide_legend(
    nrow         = 2,                  # 2 rows × many columns
    byrow        = TRUE,
    override.aes = list(size = 4)      # **one** circle per key
  )) +
  theme_void() +
  theme(
...
shared_legend = get_legend(dummy_legend_plot)

# Assemble with cowplot
row_plots  = plot_grid(p_wt, p_ko, nrow = 1, align = "hv")
...
png(file.path(geno_dir, paste0(KO2plot, "_umap.png")),
    width  = width_px,
    height = height_px,
    units  = "px",
    res    = 300,
    bg     = "transparent",  # adds alpha channel
    type   = "cairo")        # ensures RGBA output
print(umap_fig) 
dev.off()
...
png(file.path(geno_dir,paste0(KO2plot,"_ct-prop.png",sep='')),
    width  = width_px,
    height = height_px,
    units  = "px",
    res    = 300,
...
if(file.exists(file.path(de_res_dir, paste0("genotype_DEGs/", gsub("/","_",KO2plot), ".tsv")))){
  ...
  if(nrow(deg_filtered)>0){
    ...
    width_px  = 1920
    height_px = round(width_px * (2/3)) 
    png(file.path(geno_dir,paste0(KO2plot,"_nDEG-total.png",sep='')),
        width  = width_px,
        height = height_px,
        units  = "px",
        res    = 300,
    ...
    width_px  = 1920
    height_px = round(width_px * (2/3)) 
    png(file.path(geno_dir,paste0(KO2plot,"_nDEG-updown.png",sep='')),
        width  = width_px,
        height = height_px,
        units  = "px",
        res    = 300,
...
if(file.exists(file.path(de_res_dir, paste0("genotype_DEGs/", gsub("/","_",KO2plot), ".tsv")))){
  ...
  if(nrow(deg_df)>0){
    ...
    width_px  = 1920
    height_px = round(width_px * (3/4)) 
    png(file.path(geno_dir,paste0(KO2plot,"_DE-dotplot.png",sep='')),
        width  = width_px,
        height = height_px,
        units  = "px",
        res    = 300,
