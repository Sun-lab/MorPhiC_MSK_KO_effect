...
```{r}
pseudo_neurod1_expr = list()
for(tp in names(pseudo_obj_list)){
  dat = GetAssayData(pseudo_obj_list[[tp]])
  pseudo_neurod1_expr[[tp]] = dat["NEUROD1",]
}
lapply(pseudo_neurod1_expr,hist)
```
...
```{r}
"NEUROD1"%in%rownames(GetAssayData(pseudo_obj_list[[1]]))
```
